...
public:
	 ...
	 */
	BRKGA(double alpha,vector<GraphCostTS*> _g, Instance * _instance, Heuristic& _gen_sol, unsigned n, unsigned p, double pe, double pm, double rhoe,
...
private:
	...
	void evolution(Population& curr, Population& next,unsigned generations,unsigned MaxGens);
	bool isRepeated(const std::vector< double >& chrA, const std::vector< double >& chrB) const;
	void diversify(Population& next);
	
};

template <class Decoder, class RNG>
BRKGA<Decoder, RNG>::BRKGA(double _alpha,vector<GraphCostTS*> _g, Instance *_instance, Heuristic &_construtivo, unsigned _n, unsigned _p, double _pe, double _pm, double _rhoe,
						   Decoder &decoder, RNG &rng, unsigned _K, unsigned MAX) : alpha(_alpha),instance(_instance), n(_n), p(_p),construtivo(_construtivo),
...
void BRKGA< Decoder, RNG >::evolve(unsigned generations) {
	...
	for(unsigned i = 0; i < generations; ++i) {
		for(unsigned j = 0; j < K; ++j) {
			evolution(*current[j], *previous[j],i,generations);	// First evolve the population (curr, next)
			std::swap(current[j], previous[j]);		// Update (prev = curr; curr = prev == next)
		}
		//cout<<"Geracao "<<i<<": "<<getBestFitness()<<endl;
	}
}
...
template< class Decoder, class RNG >
void BRKGA< Decoder, RNG >::initialize(const unsigned i) {
	#ifdef _OPENMP
	...
	{	
		construtivo.run(alpha);
		for (unsigned k = 0; k < n; ++k)
		{
			// (*current[i])(j, k) = refRNG.randExc(1);
			
			if (construtivo.get_Solution()[k] > 0)
			{
				(*current[i])(j, k) = refRNG.randExc(0.5) +0.5;
			}else{
				(*current[i])(j, k) = refRNG.randExc(0.5);
			}
		}
	
		current[i]->setFitness(j, refDecoder.decode((*current[i])(j),0,0));
		
	}
	//cout << "Best :" << current[i]->getBestFitness() << endl;
	//getchar();
	current[i]->sortFitness();

}
template< class Decoder, class RNG >
inline void BRKGA< Decoder, RNG >::diversify(Population& next) {
	int inefficency = refDecoder.get_Ineficiency();
	int maxRepair = int(0.2*refDecoder.get_idindividuSize());
	if(inefficency > 5){
		for(int i = 0; i < maxRepair; ++i){
			int index = refDecoder.get_RandomIndex();
			//cout << "tavindo aki" << endl;
			next.setFitness( index, refDecoder.diversity(next.population[index]));
		}
	}
		
}
template< class Decoder, class RNG >
inline void BRKGA< Decoder, RNG >::evolution(Population& curr, Population& next,unsigned generations,unsigned MaxGen) {
	...
	for(int i = int(pe); i < int(p); ++i) {
		next.setFitness( i, refDecoder.decode(next.population[i],next.getBestFitness(),generations,MaxGen,i));
	}
	//diversify(next);
